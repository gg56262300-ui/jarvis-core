...
## Cursor agendi siduv töövoog (kohustuslik; omaniku AAA täpsustus)

1. **`AGENTS.md` on siduv leping.** Kui tekib kahtlus, **võidab alati** see fail; omanik ei pea reegleid kordama, et need kehtiksid.
2. **Iga omaniku sõnum enne vastust:** ära toetu „meelespidamisele“ nii, et rikuksid reegleid — rakenda vastuses järjekindlalt vähemalt **keel**, **saladused**, **runtime/PM2**, **patch/juurdepääsu piirangud** (`Cursor Patch Discipline`, `Omaniku volitus`), kui need on asjakohased.
3. **Enne esimest repo kirjutamist või riskikäsku** antud sessioonis (failimuudatused, shell, `git`, `pm2`, `npm` mis mõjutab keskkonda, `.env`-teema): tee tööriistaga **`Read` → `/AGENTS.md`** (repo juur) või hoia `AGENTS.md` juba garanteeritult kontekstis; kui oled ebakindel või kontekst on pikk, **loe uuesti** enne jätkamist.
4. **Eesmärk:** drifti vähendamine — üks kord kirjutatud reegel jääb kehtima ilma, et omanik peaks kõiki ridu käsitsi jälgima.

## Järelevalve orgaanid (kellele see töö kuulub; omanik ei pea mikrohaldusega tegelema)

Omanik ei pea **kõiki** reegleid käsitsi kontrollima — osa on **automatiseeritud**, osa on **agendi kohustus**, osa on **valve**.

1. **Git + Husky (iga `git commit` / `git push`):** `.husky/pre-commit` käivitab esmalt `scripts/agents-charter-check.sh` (kas `AGENTS.md` on olemas ja sisaldab siduvaid ankruid), seejärel `npm run gate:fast`; `.husky/pre-push` käivitab `npm run gate:full`. See on **masinnõue**: salajaste lekete skann, build, lint, testid — ilma sinu pideva järelvalveta.
2. **Jarvis valve (runtime):** `scripts/jarvis-autocheck.mjs` + `npm run health` / `pm2` voog — jälgib **teenuse elusolu**, mitte vestluse keelt; see on **operatiivorgaan**.
3. **Cursor agendi käitumine (vestlus + patch):** `AGENTS.md` siduv töövoog ülal — **LLM-i vastuse keelt** masinaga 100% tõestada ei saa; selle eest vastutab **agent** (reeglite rakendamine + vajadusel `Read` → `AGENTS.md`). Kui agent rikub, on see **agendi viga**, mitte „reeglid kadusid“.

**Käsitsi „aktiveerimine“ omanikult minimaalne:** tee tavapärane commit/push — Husky jookseb ise. Vabatahtlik kontroll: `sh ./scripts/agents-charter-check.sh`.

4. **100-punkti autonoomne ülevaatus (kood + gate'id, võrk pole vaja):** `npm run autonomic:100` → `scripts/autonomic-100-sweep.sh` (100 järjestikust kontrolli: siduvad failid, `AGENTS.md` ankurd, build, lint, vitestid, saladuste skann, 68 repo-faili olemasolu).
5. **Telegram kahesuunaline kanal (MVP+):** `GET /api/integrations/telegram/status` — diagnostika (tokenit ei näidata); `?webhook=1` lisab `getWebhookInfo` ja boti `@username` (getMe); ainult boti nime jaoks ka `?me=1`. `POST /api/integrations/telegram/webhook` — `TELEGRAM_CHAT_ID` vestlus: **tekst**, **foto caption** (ilma tekstita foto → lühike juhis), **häälsõnum** / **video_note** / **audio** / **audio-dokument** (Whisper, kui `OPENAI_API_KEY` on olemas; enne allalaadimist `record_voice` / `upload_voice`), `edited_message`; **inline-nuppe / callback_query ei kasutata**. Enne LLM-i: `typing`; pikad vastused: **mitu järjestikust sõnumit** (kuni 12 lõiku, ~4096 märki lõigu kohta), esimene `reply_to_message`. `TELEGRAM_DEFAULT_LOCALE` (vaikimisi `ru`) — Roberti süsteemviiba keel. Sissetulevate `update_id` **deduplikatsioon** (Telegrami korduskatsed). **Vaikimisi «индивидуально» Telegramis:** kui `logs/mobile-remote-state.json` on `MODE:STOP`, taastatakse **JÄTKA** vaikselt enne LLM-i iga tavalise Telegram-sõnumiga (STOP-käsk jääb eraldi tööle). **Viimane boti vastus:** pärast `sendMessage` kutsub `pinChatMessage` (vaikimisi sees, `disable_notification`); mitme lõigu puhul kinnitatakse **viimane** tükk — nähtav ülaosas ilma pika kerimiseta. Välja: `TELEGRAM_PIN_BOT_REPLY=false`. **Häälvastus:** `TELEGRAM_VOICE_REPLY=true` → OpenAI `audio/speech` (Opus) + Telegram `sendVoice` (kuni 4096 märki kõneks; pikem ülejääk tekstina); `TELEGRAM_TTS_MODEL` / `TELEGRAM_TTS_VOICE`. **LLM (mõistlikud vastused):** sissetulev Telegram läbib **sama** `processChatRequestBody` voogu mis veebi `/api/chat` — Roberti süsteemviip + tööriistad + `chat.completions`. Nõuab `OPENAI_API_KEY`; mudel `JARVIS_CHAT_COMPLETION_MODEL` (vaikimisi `gpt-4o-mini`). **Muu teenuse** (nt DeepSeek OpenAI-ühilduv) jaoks: `OPENAI_BASE_URL` + vastav võti + mudel (nt `deepseek-chat`) — tööriistade tugi sõltub teenusest. **Webhooki seadistus:** `.env`-is `JARVIS_WEBHOOK_PUBLIC_BASE` (https origina, lõpupõikuta) + `npm run telegram:set-webhook` → kutsub Telegrami `setWebhook` (valikuline `TELEGRAM_WEBHOOK_SECRET`). Soovituslik turvalisus: `TELEGRAM_WEBHOOK_SECRET` + `setWebhook` `secret_token` (päis `X-Telegram-Bot-Api-Secret-Token`). Avalik HTTPS-URL peab tunneli/VPS-i kaudu Jarvisini jõudma. **Kui küsimustele ei vastata:** sissetulev sõnum ei jõua serverisse või `401` — vaata `getWebhookInfo` (`?webhook=1`) väljasid `last_error_*`; saladus peab ühtima `setWebhook` `secret_token`iga. **Ilma tunnelita (Mac):** `TELEGRAM_USE_POLLING=true` — protsess küsib `getUpdates`, kutsub käivitamisel `deleteWebhook` ja edastab uuendused kohalikule `POST .../webhook` (sama Roberti loogika); **ära** kasuta samaaegselt avaliku URL-iga webhooki teise masina peal. **Tulevased lisakäsud (enne LLM-i):** `src/integrations/telegram/telegram-side-commands.ts` (nt `/ping`, `/jarvis_ping`, `/inbox` → `logs/agent-inbox.jsonl` viimased read); kohalik kontroll: `npm run telegram:webhook-smoke` (eeldab töötavat Jarvisit ja `.env` `TELEGRAM_CHAT_ID`). **Projektitöö käsk (ping-pong eeltingimused):** Telegramis `/jarvis` (sama tekst mis terminalis `npm run jarvis:telegram-work` — üks allikas `jarvis-telegram-workflow-text.ts`). **Koodisõna:** `TELEGRAM_INBOUND_PREFIX_REQUIRED` + `TELEGRAM_INBOUND_PREFIX` — ainult eesliitega sõnum läheb Robertile; `TELEGRAM_REPLY_SIGNATURE` — assistendi vastuse ette märgistus. **Robert DEV (Telegram):** sõnum, mis **algab** sõnaga `Robert` (nt `Robert: paranda webhook`) — salvestatakse `logs/dev-queue.jsonl` JSONL reana (`type: dev_task`, `source: telegram`, `input`, `goal`, `timestamp`), **LLM-i ei kutsuta**; tavaline voog jätkub teiste sõnumitega.
...
- **Keelade eraldamine (AAA täpsustus):** omanik võib vestluses kirjutada **mis keeles tahes** (sh vene/inglise); omaniku keele valik **ei muuda** agendi reeglit — agendi vastused jäävad **alati** eesti keeles, kuni omanik ei palu sõnaselgelt teist keelt.
...
- **AAA: Cursori töö “maksimaalne maht” (omaniku kinnitatud):** Cursori agendi töö (repo + terminal + testid + docs) tehakse **suurte RUN-pack plokkidega**, mitte 3–4 rea “pisiklikkudega”. Eesmärk on **rohkem tehtud tööd ühe kinnituse kohta** ja **paralleelselt mitmes suunas**, kui see ei riku `KINNITAN` piiranguid ega halvenda A-rada kvaliteeti.
- **AAA: omaniku käed “väljaspool Cursorit” (omaniku kinnitatud):** kui tegevus nõuab omaniku käsi väljaspool Cursorit (BotFather token, Hetzner UI, brauseri OAuth, telefoni seaded), siis on vaikimisi **1 samm korraga** (üks konkreetne tegevus + üks oodatav tulemus), et ei tekiks segadust ega “vale kleebi” olukorda.
...
- Kui vajad infot, küsi ainult 1 konkreetne asi korraga (mitte mitu küsimust).
  ...
  **Cursori sees:** üks “Run” plokk võib sisaldada **mitu loogilist sammu** (RUN-pack), kui need on **etteaimatavad** ja **ohutud** (stabiilsus+testid+tagasipööratavus). **Väljaspool Cursorit:** üks tegevus = üks samm; järgmine samm ainult pärast eelmise kinnitust või tulemust.
...
- **AAA: “3× mõtle” reegel (omaniku kinnitatud):** enne kui annad käsu või lingi, tee 3 kiiret kontrolli:
  1) **Kontekst**: Mac vs server vs brauser — kas kasutaja peab midagi klikkima väljaspool Cursorit?
  2) **Kopeeri/paste täpsus**: kas kasutajal on **üks konkreetne koht**, kust võtta väärtus (URL / code / secret) ja **üks konkreetne koht**, kuhu see panna? (väldi “võta sealt / kleebi siia”)
...
9. **Püsikäsud telefonis (Robert):**
   - **Režiim (JÄTKA / STOP):** `JÄTKA` (sh `индивидуально` / `INDIVIDUAALSELT` → sama mis `continue`) seab `MODE:JÄTKA`; `STOP` seab `MODE:STOP`. Vanu ärkamise/maganemise laadseid **spetsiaalkäske ei ole** — režiimi muudavad ainult need kaks rida.
...
16. **Mobile-mode piirid (project-chat vs kalender/meil):** kui `mobile-remote-state.json` mode = `continue` (või tulnud just `JÄTKA` / `INDIVIDUAALSELT` / `индивидуально`), siis lühikesed töövoo-küsimused (nt `mis järgmine samm`, `что дальше`, `какое следующее действие`, `next step`) tõlgendatakse **alati projekti tööseisu päringuna** (`STAATUS` vastus: A/B/C/D/E, MODE, ETAPP, version) — mitte LLM-i vabas vormis kalendri-/meiliettepanekuna. Kalendrisse või meili lisamine toimub ainult siis, kui omanik sõnaselgelt palub (nt „lisa kalendrisse”, „saada kiri”).
16.1 **Jarvisi arengufaas vs kalender:** kui sõnumis on **Jarvis/Robert** ja küsitakse **etappi/faasi/staadiumit/развития/стадии** (nt „в каком этапе проект Джарвис”) **ilma** kalendri/meili/kontakti kontekstita, vastab server **otse** sama `STAATUS`-rea loogikaga mis `STAATUS` / `JDEV` — **ei** läbi LLM-i ega kalendritööriistu. **`JDEV`** on ainult **lühikood** (sama staatuspäring mis `STAATUS` / `JARVIS PROJEKT`); kasutajal pole vaja eraldi „JDEV käsku õppida“ — võib alati kirjutada `STAATUS` või `JARVIS PROJEKT`.
17. **STAATUS kui peamine aktiveeriv käsk (lihtsustus):** telefonis on `STAATUS` vaikimisi peakäsk — ühtlasi hoiab/näitab projekti seisu ja näitab kohe tervise kokkuvõtet.
